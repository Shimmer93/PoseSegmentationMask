...
def write_psm(save_path, joint_masks, body_mask=None, obj_mask=None, rescale_ratio=1.0):
    ...
    out_masks = (out_masks > 0.5).astype(np.uint8)
...
class CocoMetricPSM(BaseMetric):
    ...
    def process(self, data_batch: Sequence[dict],
        ...
        """
        # print(data_samples[0]['pred_fields'].keys())
        for data_sample in data_samples:
            ...
            if 'pred_fields' in data_sample:
                ...
                masks = data_sample['pred_fields']['heatmaps']
                body_mask = masks[0].cpu().numpy()
                joint_mask = masks[1:].cpu().numpy()
                write_psm(f'{self.outfile_prefix}_{id}_{img_id}_{category_id}.png', joint_mask, body_mask, rescale_ratio=4.)
                # self.result_psms.append(data_sample['pred_fields']['heatmaps'])
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        # self.results2psms(valid_kpts, outfile_prefix=outfile_prefix)
        # for img_id, instances in valid_kpts.items():
        #     for instance in instances:
        #         del instance['masks']
    ...
    def results2psms(self, keypoints: Dict[int, list],
                     outfile_prefix: str) -> str:
        for _, img_kpts in keypoints.items():
            for i, instance in enumerate(img_kpts):
                ...
                body_mask = masks[0].cpu().numpy()
                joint_mask = masks[1:].cpu().numpy()
                write_psm(f'{outfile_prefix}_{img_id}_{category_id}_{i}.png', joint_mask, body_mask, rescale_ratio=4.)
